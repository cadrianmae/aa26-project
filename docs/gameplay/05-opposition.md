...
The rival hive is a `CommanderAI`: its own Matriarch, its own swarm of
Thargoid Drones, the same five intents, the same behaviours. Built from the same
...
onto Matriarch and swarm: move, fire, set intent, designate. The ship does
not know which is at the helm.

Three things follow from that:

**The rival hive is testable.** Point `CommanderAI` at the player's own
Matriarch and it flies it. Any bug in the AI shows up immediately, in a ship
...
Destroy the rival hive's Matriarch. Losing your own ends the match.

Nothing else is a win condition. Drones are ammunition, not territory. There
is no base to raze, no points to accumulate, and no timer.

That single condition is what keeps the economy honest: every Drone you grow
exists to protect one Matriarch or threaten another, and any strategy that
...
model that. Both Matriarchs use a single health pool with uniform damage:
...
[DRAFT — Mae: worth deciding whether the rival hive's Matriarch flees at low
